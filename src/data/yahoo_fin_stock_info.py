...
def get_quote_table(ticker, dict_result=True):
    ...
    return dict(zip(data.attribute, data.value)) if dict_result else data
...
def tickers_sp400(include_company_data=False):
    ...
    return [x.replace(".", "-") for x in tickers_lst]
...
def clean(list):
    return [l for l in list if l.isalpha() == True]
...
def tickers_finviz():
    ...
    return clean(signals_ticker_list)
...
def tickers_top_crypto():
    ...
    session.close()
    return sorted(screener_df["Symbol"])
...
def get_earnings(ticker):
    ...
    result = {
        "quarterly_results": pd.DataFrame.from_dict(
...
def get_next_earnings_date(ticker):
    ...
    new_url = f"{base_earnings_url}/" + ticker
...
def get_earnings_for_date(date, offset=0, count=1):
    ...
    return current_earnings + more_earnings
